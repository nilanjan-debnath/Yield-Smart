...
export default function ShowMessage({ data, index, setEdit, conversationId, showDateTime, db, setAllChat, handleInputAnswer }) {
            ...
            <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseEnter} className={`flex justify-end items-center gap-2 relative ${editShow ? 'mb-12' : ''}`}>
                ...
                <div className={`${editShow ? '' : 'hidden'} editBox absolute right-0 top-0 w-full h-full`}>
                    ...
                    <div className="flex gap-2 float-end">
                        <button onClick={handleClose} className="px-4 py-2 w-20 rounded-full border-blue-500 border-2 text-blue-500">Close</button>
                        <button disabled={loading} onClick={handleSend} className="px-4 py-2 w-20 rounded-full bg-blue-500 text-white">{loading? 'Saving..' : 'Send'}</button>
                    </div>
                </div>
            </div>
            {data.image != '' && (
                <div className="flex justify-end items-center gap-2">
                    <div className="w-20 h-20 overflow-hidden border-2 rounded-md relative">
                        ...
                        <button onClick={() => imageRef.current.click()} className={`absolute right-0 bottom-0 p-2 bg-gray-200 rounded-full ${editShow ? '' : 'hidden'}`}><FcEditImage className='text-xl' /></button>
                    </div>
                </div>
            )}
